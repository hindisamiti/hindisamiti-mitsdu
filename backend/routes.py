...
@route.route('/api/events/<event_id_or_slug>', methods=['GET'])
def get_public_event_details(event_id_or_slug):
    """Public endpoint to get specific event details by ID or Slug"""
    try:
        # Try to lookup by ID first (if integer)
        if event_id_or_slug.isdigit():
           event = Event.query.filter_by(id=int(event_id_or_slug)).first()
        else:
           # Lookup by slug
           event = Event.query.filter_by(slug=event_id_or_slug).first()
...
# ==================== SHARE / LINK PREVIEW ROUTES ====================

@route.route('/api/share/blogs/<blog_id_or_slug>', methods=['GET'])
def share_blog(blog_id_or_slug):
    """
    Serve a static HTML page with Open Graph tags for social media crawlers,
    then redirect users to the actual frontend page.
    """
    try:
        # Lookup blog
        if blog_id_or_slug.isdigit():
            blog = Blog.query.get(int(blog_id_or_slug))
        else:
            blog = Blog.query.filter_by(slug=blog_id_or_slug).first()
            
        if not blog:
            return "Blog not found", 404

        # Frontend URL (Hardcoded based on user input, or env var)
        frontend_url = os.environ.get('FRONTEND_URL', 'https://hindisamiti-mitsdu.onrender.com')
        target_url = f"{frontend_url}/blogs/{blog.slug or blog.id}"
        
        # Prepare Metadata
        title = f"{blog.title} | Hindi Samiti"
        description = (blog.content[:150] + '...') if blog.content else "Read this blog on Hindi Samiti."
        image = blog.cover_image_url
        if image and not image.startswith('http'):
            # Make image URL absolute for the crawler
            # Assuming backend and frontend usage, we can try to guess the backend URL or use the frontend one if it's external
            # Ideally, this should be the full backend URL for uploaded images
            pass 

        # Simple HTML template with OG tags and JS redirect
        html = f"""
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            {f'<meta property="twitter:image" content="{image}">' if image else ''}
...
@route.route('/api/share/events/<event_id_or_slug>', methods=['GET'])
def share_event(event_id_or_slug):
    """
    Serve a static HTML page with Open Graph tags for events.
    """
    try:
        if event_id_or_slug.isdigit():
            event = Event.query.filter_by(id=int(event_id_or_slug)).first()
        ...
        frontend_url = os.environ.get('FRONTEND_URL', 'https://hindisamiti-mitsdu.onrender.com')
        target_url = f"{frontend_url}/events/{event.slug or event.id}"
        
        title = f"{event.name} | Hindi Samiti"
        description = (event.description[:150] + '...') if event.description else "Check out this event!"
...
@route.route('/api/blogs/<blog_id_or_slug>', methods=['GET'])
def get_public_blog_details(blog_id_or_slug):
    """Public endpoint to get a single blog by ID or Slug"""
    try:
        # Try to lookup by ID first (if integer)
        if blog_id_or_slug.isdigit():
            blog = Blog.query.get(int(blog_id_or_slug))
